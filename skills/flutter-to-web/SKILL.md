...
服务对象是熟悉 Vue3、React、正在接手途强 Flutter 项目的前端开发者。不要停留在 Dart 语法翻译；默认从一个用户可感知的操作开始，把事件如何触发、数据如何获取、状态如何保存、依赖如何通知、UI 如何重建讲成可核验的完整闭环。

## 1. 项目根目录与事实来源

先按 [动态项目根目录协议](references/project-root-resolution.md) 得到并验证 `<TUQIANG_ROOT>`，后续命令、路径和证据都基于它。不得假定项目位于某个盘符、用户目录或固定绝对路径。
...
复杂链路先读 [完整业务链追踪](references/full-flow-tracing.md)；涉及 Riverpod 时再读 [状态与 Riverpod](references/state-and-riverpod.md)。按当前问题裁剪范围，但必须闭环：

1. **操作顶点**：确定 App 启动、路由进入、生命周期、点击、输入、下拉刷新、Timer、推送或插件回调中的真实起点。
2. **入口接线**：找到 Widget 回调、route builder、Host/Coordinator、`ProviderScope` override 及跨 Feature contract；只保留会改变当前链路的接线。
...
5. **状态依赖链**：找到 State、Notifier/Controller、Provider/family key、派生 Provider、override 和非 Riverpod 状态载体。
...
Flutter + Riverpod 的异步流程通常不是一条始终存在的同步栈。回答必须明确分开并最终按时间拼合：

```text
事件/同步调用链：用户操作 → Widget 回调 → Command/Notifier → Repository 调用
异步数据链：     请求发出 → await 返回 → DTO/Model → State/缓存写入
响应式重建链：   Provider 通知 → 派生状态重算 → Widget build → 字段展示
```
...
每个影响答案的 Riverpod Provider 或其他状态载体都必须回答：

1. **定义**：声明、完整泛型、State、Notifier/Controller 在哪里；
2. **参数**：route、family、action 参数各从哪里产生，各自控制什么；
3. **实例身份**：为什么 `.family` 能传参，key 如何参与实例选择/缓存，相同与不同 key 有何区别；对象 key 的不可变性及 `==/hashCode` 是否可靠；
4. **依赖**：builder/build 中 `watch/read` 了什么，override 的具体实现从哪里注入；
5. **写入**：初始值和 `state =`、`copyWith`、seed/apply/update、Manager/cache setter、`setState/ValueNotifier` 等所有相关写入；
6. **读取与消费**：`watch/select/listen/read` 或 getter 分别在哪里，最终哪个 Widget 使用哪个字段；
...
| [references/state-and-riverpod.md](references/state-and-riverpod.md) | family、参数身份、读写、派生状态和生命周期 |
...
- [ ] 是否覆盖 Manager/cache、`setState`、`ValueNotifier` 和插件等真实旁路？
