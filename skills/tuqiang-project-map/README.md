...
`tuqiang-project-map` 是途强三端 Flutter monorepo 的项目事实、架构分层与跨文件索引 skill，版本 `1.1.0`。它按仓库身份动态发现 `<TUQIANG_ROOT>`，不绑定某台电脑的安装路径。
...
- 为 `flutter-to-web` 提供教学所需的事实证据，为 `tuqiang-dev` 提供代码归属、现有抽象与同层风格样本的索引。

## 使用边界

本 skill 不做 Vue3/React 教学，也不自行决定实现方案或修改代码。需要前端类比时由 `flutter-to-web` 消费本 skill 的证据，需要开发、修复和验证时由 `tuqiang-dev` 基于本 skill 的归属与复用信息做最终取舍。
