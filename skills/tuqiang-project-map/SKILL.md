...
description: 为途强 Flutter monorepo 的 Tuqiang 与 Laoying 两条产品线提供可核验的仓库事实、模块 owner、状态/数据拓扑及平台、地图后端、设备页面变体索引；当架构、路由、状态、跨模块依赖、业务链或变更影响面需要先按产品线定位时使用。负责证据与归属依据，不承担 Vue3/React 教学、实现方案决策或代码修改。
license: Apache-2.0
metadata:
  version: "1.3.0"
---
...
- `cross-product`：问题明确涉及两条产品线、差异比较，或当前调用证据证明 core/plugin 公共能力的变更会同时影响两产品；分别建立证据链，不能把其中一条当作另一条的实现。文件位于 core/plugin 本身不自动扩大产品范围。
...
- 调查开发影响面时，必须把产品、平台、宿主 target、地图实现、设备类型/scene 与最终 route/Page 分开枚举；一个 `standard` target、一个地图 enum 或一个已打开页面都不能代表全部变体。
...
- 设备列表 → 设备首页/详情 → “更多详情/更多设置”存在多个 `deviceType`、scene、cameraScene 或服务状态分支，而用户没有说明是全部设备还是指定类型；
...
4. 需求或 Bug 可能跨客户端、地图或设备详情族时，按 [平台、地图后端与设备页面变体](references/variant-surface.md) 建立候选矩阵；设备/普通业务只列当前可达叶子，Tuqiang 主地图保留 7 个候选行，任何 `无需修改` 都要有源码证据。
5. 调查开发归属时，确认当前 owner 和依赖方向，再查接口、基类、helper、Provider/ChangeNotifier、Repository、组件、同层实现和测试。
6. 遇到 app composition callback、route registry、runtime callback 或 controller 时，继续追到最终 builder/owner；不要在 contract 停止。
7. 向教学任务返回调用/数据/状态证据；向开发任务额外返回 owner、现有复用入口、同层样本、依赖边界、资源边界与变体矩阵。本 skill 不做最终实现决策。
...
| [references/device-location-flow.md](references/device-location-flow.md) | 仅当 Tuqiang 问题涉及设备目录、选择设备、定位状态，或涉及设备/详情路由的 GPS UI 时读取 |
| [references/variant-surface.md](references/variant-surface.md) | 开发/修复/评审可能跨平台、地图源/后端、设备类型、详情页或“更多详情”时，枚举当前可达变体 |
...
- [ ] 涉及变更影响面时，已拆开平台/target、地图后端与设备 route leaf，没有以第一个命中代表全部实现。
